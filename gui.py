...
from PySide2.QtWidgets import (QApplication, QMainWindow, QMessageBox, 
                             QLabel, QVBoxLayout, QWidget) # 추가: 시각화 표시용
from PySide2.QtCore import Slot, QThread, Signal
from PySide2.QtGui import QFont, QPixmap # 추가: 이미지 표시용

# 데이터 처리 및 시각화 라이브러리
import pandas as pd
import matplotlib.pyplot as plt

# 사용자 UI 파일 (반드시 존재해야 함)
from ui_form import Ui_MainWindow

# --- 환경설정 ---
# ⚠️ 주의: 실제 키를 사용하고 안전하게 관리하세요.
OPENAI_API_KEY = "key" # ⬅️ 실제 키로 변경하세요!
FIREBASE_CRED_PATH = "AGVprojectFirebase.json"
MQTT_BROKER_ADDRESS = "192.168.68.45"
MQTT_PORT = 1883
MQTT_COMMAND_TOPIC = "agv/command"
MQTT_SENSING_TOPIC = "agv/sensing"
# --- 환경설정 끝 ---
...
class MainWindow(QMainWindow):
    ...
    def execute_and_show_plot(self, code_string, data_df):
        ...
        try:
            # 폰트 설정 시도 (없으면 무시)
            try:
                plt.rcParams['font.family'] = 'NanumGothic' # 또는 'Malgun Gothic' 등 시스템에 설치된 폰트
                plt.rcParams['axes.unicode_minus'] = False
            except Exception:
                self.ui.logText.appendPlainText("경고: 한글 폰트 설정에 실패했습니다. 차트의 한글이 깨질 수 있습니다.")

            # ⚠️⚠️⚠️ 보안 경고: exec()는 매우 위험합니다! ⚠️⚠️⚠️
            ...
            if self.plot_window and self.plot_window.isVisible():
                self.plot_window.close() # 기존 창 닫기
            ...
            plt.close('all') # 모든 Matplotlib 창 닫기 (메모리 누수 방지)
            buffer.close()
    ...
    def closeEvent(self, event):
        ...
        if self.plot_window and self.plot_window.isVisible():
             self.plot_window.close() # 차트 창도 함께 닫기
        event.accept()

if __name__ == "__main__":
    # ⚠️ ui_form.py 파일이 없으면 이 부분에서 오류가 발생합니다.
    #    Qt Designer로 UI를 만들고 pyside2-uic로 변환하여 사용해야 합니다.
    try:
        from ui_form import Ui_MainWindow
    except ImportError:
    ...
    # 한글 폰트 설정 (GUI 전체에 적용) - 시스템에 맞는 폰트로 변경 가능
    try:
        font = QFont("NanumGothic", 10) # ⬅️ 시스템에 설치된 한글 폰트로 변경!
        app.setFont(font)
    except Exception as e:
