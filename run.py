...
# --- Firebase Configuration (⚠️ 사용자 수정 필요) ---
# 프로젝트 1: RTDB 용
RTDB_SERVICE_ACCOUNT_KEY_PATH = 'mid-pjt-fec47-firebase-adminsdk-fbsvc-ff433d1b6e.json' # ◀️ RTDB 프로젝트 키 경로로 수정
RTDB_DATABASE_URL = 'https://mid-pjt-fec47-default-rtdb.firebaseio.com/' # ◀️ RTDB 프로젝트 URL로 수정
RTDB_APP_NAME = 'rtdbApp' 

# 프로젝트 2: Firestore 용
FIRESTORE_SERVICE_ACCOUNT_KEY_PATH = 'AGVprojectFirebase.json' # ◀️ Firestore 프로젝트 키 경로로 수정
FIRESTORE_APP_NAME = 'AGVproject'

# RTDB 경로
DISTANCE_DB_PATH = '/esp32/DATAS/DISTANCE'
HUMID_DB_PATH = '/esp32/DATAS/HUMID'
TEMP_DB_PATH = '/esp32/DATAS/TEMP'

# Firestore 컬렉션 이름
FIRESTORE_COLLECTION_NAME = 'agv_sensor_logs' 
...
def upload_to_firestore(collection_name, document_id, data):
    ...
    try:
        ...
        # print(f"Data successfully set to Firestore. Document ID: {document_id}") # 성공 로그
    except Exception as e:
...
# --- MQTT Configuration ---
MQTT_BROKER_HOST = "192.168.68.45" # ◀️ 사용자 환경에 맞게 수정
    ...
    def run(self):
        ...
        while self.th_flag:
            try:
                ...
                # else: # Firestore 꺼져있을 때 로그 (선택 사항)
                #    print("SensingThread: Firestore client not ready. Skipping Firestore upload.")
                ...
                # else: # MQTT 꺼져있을 때 로그 (선택 사항)
                #     print("SensingThread: MQTT not connected. Skipping MQTT publish.")
...
# --- MQTT 시작 여부와 관계없이 실행되도록 주석 처리 또는 제거 ---
# init_mqtt_client_agv() 
# ⬆️ 만약 MQTT를 사용하지 않으려면 이 라인을 주석 처리하거나, 
#    별도의 'MQTT 시작' 버튼을 만들거나, 
#    SensingThread 내부의 MQTT 관련 코드에 더 많은 예외 처리를 추가할 수 있습니다.
#    여기서는 일단 호출은 하되, SensingThread 내부에서 연결 상태를 확인하도록 합니다.
init_mqtt_client_agv()
